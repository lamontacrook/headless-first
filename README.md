...
Our model type will be Image List.  Give the fragment a title and name.

2.  Since and image list is a container type fragment that includes reference to other fragments, we can create our offers from the editor.  Create a new offer fragment an include and image description and article.  You may cut and paste text from below.  We will be using the title of the fragment as the title of the card.
...
"When rare sunrays pierce through the low clouds, the scenery revelas its vibrancy"
...
Let's take a look at our imagelist component.  Inside the imagelist we loop through each offer.
...
In the card component we will render each card.
